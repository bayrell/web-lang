...
Object.assign(Runtime.rtl,
{
	...
	runApp: async function(module_name, class_name, d)
	{
		...
		d = Runtime.rtl.setAttr(d, Runtime.Collection.from(["modules"]), Runtime.Collection.from([module_name]));
		try
		{
	...
	NativeToObject: function(value)
	{
		...
		var value2 = this.PrimitiveToObject(value1);
		return value2;
	},
	/**
	...
	PrimitiveToObject: function(obj)
	{
		...
		if (obj instanceof Runtime.Collection)
		{
			...
			for (var i = 0;i < obj.count();i++)
			{
				...
				value = this.PrimitiveToObject(value);
				res.pushValue(value);
			}
	...
	json_decode: function(obj)
	{
		try{
			...
			return this.PrimitiveToObject(res);
		}
		catch(e){
			throw e;
		}
...
Object.assign(Runtime.rs,
{
	...
	strip_tags: function(content, allowed_tags)
	{
		...
		{
			return Runtime.re.replace("<[^>]+>", "", content);
		}
...
Object.assign(Runtime.Exceptions.RuntimeException.prototype,
{
	...
	assignValue: function(k,v)
	{
		if (k == "error_str")this.error_str = v;
	...
	{
		if (d == undefined) d = null;
		if (k == "error_str")return this.error_str;
...
Object.assign(Runtime.BaseObject,
{
	 ...
	 * Init struct data
	 */
	_init_data: function(old, changed)
	{
		return changed;
	},
	/**
	...
	_assign: function(new_item, old_item, obj)
	{
		...
		obj = new_item.constructor._init_data(old_item, obj);
		if (obj == null)
		{
			return ;
		}
		var check_types = false;
		var class_name = new_item.constructor.getClassName();
		var _Dict = use("Runtime.Dict");
		...
		else
		{
			for (var key in obj)
			{
				...
				if (check_types)
				{
					info = rtl.getFieldInfoWithParents(new_item.getClassName(), key);
					if (info)
					{
						value = rtl.convert(value, info.get("t"), null);
					}
