...
Object.assign(Bayrell.Lang.LangBay.ParserBayOperator,
{
	...
	readDeclareFunction: function(parser, has_name)
	{
		...
		parser = Runtime.rtl.setAttr(parser, Runtime.Collection.from(["vars"]), save_vars);
		parser = Runtime.rtl.setAttr(parser, Runtime.Collection.from(["is_html"]), save_is_html);
		return Runtime.Collection.from([parser,new Bayrell.Lang.OpCodes.OpDeclareFunction(Runtime.Dict.from({"args":args,"vars":vars,"flags":flags,"name":name,"is_html":is_html,"is_context":is_context,"result_type":result_type,"expression":expression,"items":op_code,"caret_start":caret_start,"caret_end":parser.caret}))]);
	},
	/**
...
Object.assign(Bayrell.Lang.LangES6.TranslatorES6Html,
{
	...
	OpHtmlAttrs: function(t, attrs, item_pos)
	{
		...
		for (var attrs_i = 0;attrs_i < attrs.count();attrs_i++)
		{
			...
			{
				attr_key = "@model";
				attr_value = "[null," + Runtime.rtl.toStr(attr_value) + Runtime.rtl.toStr("]");
			}
		...
		{
			attr_class.pushValue("Runtime.rs.getCssHash(this.getClassName())");
		...
		for (var i = 0;i < attrs.count();i++)
		{
			...
			attr_s = "Runtime.rs._merge_attrs(" + Runtime.rtl.toStr(attr_s) + Runtime.rtl.toStr(",") + Runtime.rtl.toStr(attr.value.value) + Runtime.rtl.toStr(")");
		}
...
Object.assign(Bayrell.Lang.LangPHP.TranslatorPHPHtml,
{
	...
	OpHtmlComponent: function(t, op_code)
	{
		...
		var attr_global = op_code.attrs.findItem(Runtime.lib.equalAttr("key", "@global"));
		if (attr_name)
		{
			var res = t.expression.constructor.Expression(t, attr_name.value);
			t = Runtime.rtl.get(res, 0);
			v_model = "\\Runtime\\rs::_concat_attrs($ctx, $model_path, " + Runtime.rtl.toStr(Runtime.rtl.get(res, 1)) + Runtime.rtl.toStr(")");
		}
		else if (attr_bind)
		{
		...
		{
			var res = t.expression.constructor.Expression(t, attr_global.value);
			t = Runtime.rtl.get(res, 0);
			v_model = "\\Runtime\\rs::_concat_attrs($ctx, [], " + Runtime.rtl.toStr(Runtime.rtl.get(res, 1)) + Runtime.rtl.toStr(")");
		}
	...
	OpHtmlAttrs: function(t, attrs)
	{
		...
		var res_attrs = attrs.map((attr) => 
		{
			...
			return attr_key + Runtime.rtl.toStr("=\"'.\\Runtime\\rs::_escape_attr($ctx, ") + Runtime.rtl.toStr(attr_value) + Runtime.rtl.toStr(").'\"");
		});
		res_attrs = res_attrs.filter((s) => 
		{
			return s != "";
		...
		for (var i = 0;i < attrs.count();i++)
		{
			...
			attr_s += Runtime.rtl.toStr(" ' . \\Runtime\\rs::_join_attrs($ctx, $" + Runtime.rtl.toStr(attr.value.value) + Runtime.rtl.toStr(") . '"));
		}
		return Runtime.Collection.from([t,attr_s]);
	},
	/**
	...
	OpHtmlTag: function(t, op_code)
	{
		...
		else
		{
			...
			if (op_code.tag_name != "")
			{
				content += Runtime.rtl.toStr(t.s("\\Runtime\\rs::_p(" + Runtime.rtl.toStr(var_name) + Runtime.rtl.toStr(", '</") + Runtime.rtl.toStr(op_code.tag_name) + Runtime.rtl.toStr(">');")));
			}
		}
		var res = t.constructor.addSaveOpCode(t, Runtime.Dict.from({"op_code":op_code,"var_name":var_name,"content":content}));
		t = Runtime.rtl.get(res, 0);
		...
		for (var i = 0;i < op_code.items.count();i++)
		{
			...
			else if (item instanceof Bayrell.Lang.OpCodes.OpHtmlValue)
			{
				...
				else if (item.kind == Bayrell.Lang.OpCodes.OpHtmlValue.KIND_HTML)
				{
					...
					t = Runtime.rtl.get(res, 0);
					item_value = Runtime.rtl.get(res, 1);
					item_value = "\\Runtime\\rs::_to_html($ctx, " + Runtime.rtl.toStr(item_value) + Runtime.rtl.toStr(")");
				}
			...
			else if (item instanceof Bayrell.Lang.OpCodes.OpCall)
			{
				...
				t = Runtime.rtl.get(res, 0);
				item_value = Runtime.rtl.get(res, 1);
				item_value = "\\Runtime\\rs::_escape_html($ctx, " + Runtime.rtl.toStr(item_value) + Runtime.rtl.toStr(")");
			}
			else
			{
				var res = t.expression.constructor.Expression(t, item);
			...
			if (item_value != "")
			{
				content += Runtime.rtl.toStr(t.s("\\Runtime\\rs::_p(" + Runtime.rtl.toStr(var_name) + Runtime.rtl.toStr(", ") + Runtime.rtl.toStr(item_value) + Runtime.rtl.toStr(");")));
			}
		}
...
Object.assign(Bayrell.Lang.LangPHP.TranslatorPHPOperator,
{
	...
	Operators: function(t, op_code)
	{
		...
		else if (op_code instanceof Bayrell.Lang.OpCodes.OpHtmlItems)
		{
			...
			if (Runtime.rtl.get(res, 1) != "")
			{
				content += Runtime.rtl.toStr(t.s("\\Runtime\\rs::_p(" + Runtime.rtl.toStr(save_html_var_name) + Runtime.rtl.toStr(", ") + Runtime.rtl.toStr(Runtime.rtl.get(res, 1)) + Runtime.rtl.toStr(");")));
			}
			t = Runtime.rtl.setAttr(t, Runtime.Collection.from(["save_op_codes"]), save_op_codes);
			t = Runtime.rtl.setAttr(t, Runtime.Collection.from(["save_op_code_inc"]), save_op_code_inc);
			t = Runtime.rtl.setAttr(t, Runtime.Collection.from(["is_html"]), save_is_html);
